Try 
{
  ...
  If (!(($vers.Major -eq 10) -or (($vers.Major -eq 6) -and ($vers.Minor -ge 2)))) { 
    [System.Windows.Forms.MessageBox]::Show("This operating system is not supported. This script only supports Windows NT 6.2, 6.3 or 10.x. (i.e. Windows 8, Windows Server 2012, Windows 8.1, Windows Server 2012 R2 or Windows 10). You seem to be running:`r`r"+[System.Environment]::OSVersion.VersionString, "Script", "OK", "Error");
    break;
  }
  
  # Try to get number of transcoded images
  try {
		$TranscodedImageCount=(Get-ItemProperty -Path "HKCU:\Control Panel\Desktop" -Name TranscodedImageCount).TranscodedImageCount
  }
  catch [System.Management.Automation.ItemNotFoundException],[System.Management.Automation.PSArgumentException]  {
    [System.Windows.Forms.MessageBox]::Show("Windows does not seem to be holding a record of a wallpaper at this time.`r`r"+$Error[0].Exception.Message,"Script","OK","Error");
    break;
  }
  
  for ($j = 0; $j -lt $TranscodedImageCount; $j++) {
	  ...
	  $transImage = "TranscodedImageCache_00$j" 
	  
	  # First, access Windows Registry and get the property containing wallpaper path
	  try {
			$TranscodedImageCache=(Get-ItemProperty -Path "HKCU:\Control Panel\Desktop" -Name $transImage).$transImage
	  }
	  catch [System.Management.Automation.ItemNotFoundException],[System.Management.Automation.PSArgumentException]  {
			[System.Windows.Forms.MessageBox]::Show("Windows does not seem to be holding a record of a wallpaper at this time.`r`r"+$Error[0].Exception.Message,"Script","OK","Error");
			break;
	  ...
	  try {
			...
			[System.Windows.Forms.MessageBox]::Show("Wallpaper is not found at the location Windows believes it is: `r$WallpaperSource", "Script", "OK", "Warning");
			break;
	  }  
	  
	  # Wallpaper should by now have been found. Present it to the user.
  ...
  [System.Windows.Forms.MessageBox]::Show("Error!`r`r"+$Error[0], "Script", "OK", "Error");
  break;
}